	...
	*/
	...
	void CBloomRender::Create( const SGraphicsConfig& config )
	{
		if (config.bloomConfig.isEnable) {
			...
			m_downSamplingRenderTarget[0].Create(w >> 2, h, 1, FMT_A16B16G16R16F, FMT_INVALID, MULTISAMPLE_NONE, 0);			//横ブラー用。
			m_downSamplingRenderTarget[1].Create(w >> 2, h >> 2, 1, FMT_A16B16G16R16F, FMT_INVALID, MULTISAMPLE_NONE, 0);	//縦ブラー用。
			m_downSamplingRenderTarget[2].Create(w >> 3, h >> 2, 1, FMT_A16B16G16R16F, FMT_INVALID, MULTISAMPLE_NONE, 0);			//横ブラー用。
			m_downSamplingRenderTarget[3].Create(w >> 3, h >> 3, 1, FMT_A16B16G16R16F, FMT_INVALID, MULTISAMPLE_NONE, 0);	//縦ブラー用。
