...
namespace tkEngine{
	...
	void CShadowMap::Update()
	{
		...
		if (fabsf(cameraForward.y) < 0.01f) {
			return;
		}
		CVector3 cameraUp;
		...
		cameraUp = CVector3::Up;
		//ライトビュー行列の回転成分をを計算する。
		CVector3 lightViewForward = m_lightDirection;
		//ライトビューの上を決める。
		CVector3 lightViewRight;
		lightViewRight.Cross(m_lightDirection, cameraRight);
		lightViewRight.Normalize();
		
		CVector3 lightViewUp;
		lightViewUp.Cross(lightViewRight, lightViewForward);
		...
		lightTarget += cameraForward * shadowAreaTbl[0] * 0.5f;
		...
		for (int i = 0; i < NUM_SHADOW_MAP; i++) {
			...
			CVector3 v[8];
			{
				...
				//近平面の中央座標を計算。
				CVector3 vWk = MainCamera().GetPosition() + cameraForward * nearPlaneZ;
				v[0] = vWk + cameraRight * t * nearPlaneZ + toUpperNear;
				v[1] = v[0] - toUpperNear * 2.0f;

				v[2] = vWk + cameraRight * -t * nearPlaneZ + toUpperNear;
				v[3] = v[2] - toUpperNear * 2.0f;

				//遠平面の中央座標を計算。
				vWk = MainCamera().GetPosition() + cameraForward * farPlaneZ;
				v[4] = vWk + cameraRight * t * farPlaneZ + toUpperFar;
				v[5] = v[4] - toUpperFar * 2.0f;
				v[6] = vWk + cameraRight * -t * farPlaneZ + toUpperFar;
			...
			lightPos += cameraForward * shadowAreaTbl[i] * 0.5f;
			if (i < NUM_SHADOW_MAP - 1) {
				lightPos += cameraForward * shadowAreaTbl[i + 1] * 0.5f;
			}
